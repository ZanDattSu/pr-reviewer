...
#### Реализован дополнительный эндпоинт `/users/deactivate`
Деактивирует переданных пользователей (is_active = false) и для всех открытых PR,
где они были назначены ревьюверами, пытается найти активную замену из их команды.
...
Подробности так же можно посмотреть в openApi документации
...
- Действия: 
  - Начал изучать возможные решения и наткнулся на статью на хабре про ваш [менеждер транзакций](https://habr.com/ru/companies/avito/articles/727168/). Изучив принципы работы, принял решение, что это именно то, что нужно для решения моей проблемы. 
  ...
  - Можно реализовывать всю бизнес логику в сервисном слое, без создания кучи дополнительных методов в сервисном слое.
  - Убирает кучу дублирующегося кода
