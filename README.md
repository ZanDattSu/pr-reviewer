...
1.
- Проблема:
   - При запуске сервиса через Docker Compose и попытке обратиться к HTTP-эндпойнту (например, /health) с хоста по адресу localhost:8080, клиент (Postman) получает ошибку “socket hang up”. 
     Контейнер успешно стартует, порты проброшены (8080:8080), но доступ извне отсутствует. 
- Причина:
  - В Go‑коде сервис запускался с привязкой к адресу localhost:8080.
    В Docker‑контейнере это означает, что сервер слушает только локальные соединения внутри контейнера, а обращения снаружи (по проброшенному порту) не принимаются.
- Решение:
  - В качестве адреса для прослушивания указал 0.0.0.0:8080. 
    Это заставило приложение слушать на всех интерфейсах, проброс порта из контейнера на хост заработал, и сервис стал доступен извне по localhost:8080.
